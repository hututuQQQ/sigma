...
<p align="center">
  一个支持持久会话、证据闭环与统一 Pi 模型网关的 Coding Agent。<br>
  通过终端或 Sigma Code 桌面端完成规划、分析、修改、执行、验证、审查与恢复。
</p>
...
<p align="center">
  <a href="https://github.com/hututuQQQ/sigma/releases/tag/v0.1.4"><img alt="状态：0.1.4 稳定版" src="https://img.shields.io/badge/status-0.1.4%20stable-2ea44f"></a>
...
Sigma Code 把一次编码任务变成一条可持久化、可验证、可重放的类型化事件流。它可以理解仓库、执行范围明确的修改、在沙箱内运行命令、验证结果、请求独立审查，并在进程中断后恢复原来的会话。本仓库维护 Sigma Runtime、CLI 与 TUI；桌面、Web 和移动客户端位于 [hututuQQQ/sigma-code](https://github.com/hututuQQQ/sigma-code)。终端界面使用进程内 `RuntimeClient`，桌面客户端内置的 Sigma Provider 则通过稳定的 ACP v1 驱动同一个事件溯源内核、会话仓库、工具、恢复逻辑和结果协议。

`0.1.4` 是当前受支持的产品基线。Linux x64 是稳定发布目标；
...
> **当前产品边界**
>
> - **[Sigma Code 0.1.4](https://github.com/hututuQQQ/sigma/releases/tag/v0.1.4) 在 Linux x64 上是稳定版；Windows x64 是未签名预览版。** Windows 安装包同时包含 Sigma Code 桌面 UI 和经过验证的 Sigma Runtime，用户无需另外安装 Node.js 或 Agent CLI。发布候选必须通过原生沙箱、打包产品、校验和、SBOM 与签名来源证明门禁；Windows 可执行文件目前没有可信 Authenticode 签名，可能触发 Windows 安全警告。
...
| 一个 Runtime，多种界面 | CLI 自动化和 TUI 共用同一个 `RuntimeClient`；`sigma acp` 通过稳定的 ACP v1 把同一 Runtime 暴露给 Sigma Code 桌面客户端，不复制第二套 Agent。 |
...
本仓库发布的 Windows 安装包会把客户端与同一 Release 中经过验证的 Sigma Runtime 打包在一起。客户端内置的 Sigma Provider 会启动长驻的 `sigma acp` Server，并通过 stdio 上的换行分隔 JSON-RPC 通信。v0.1.4 的 ACP Bridge 支持：
...
从经过验证的项目 Release 获取 `0.1.4` Linux x64 稳定版归档，或从源码构建；
...
如需完整桌面产品，请从 [Sigma Code v0.1.4 Release](https://github.com/hututuQQQ/sigma/releases/tag/v0.1.4)
下载 `Sigma-Code-0.1.4-x64.exe`，核对 SHA-256 侧文件后运行安装程序。
...
公开验证密钥。Linux x64 在 `0.1.4` 是稳定发布；Windows x64 仍属于未签名预览。
