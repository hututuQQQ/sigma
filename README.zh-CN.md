...
配置 Schema v4 默认使用 `sandbox=required`、`read_scope=host`、`network=full` 和 `process_handoff=allow`。放宽的只有已声明只读路径和 Socket 能力：工作目录与所有写入仍限制在工作区，必需沙箱绝不会回退到不安全的宿主机执行。`ask` 模式对 `filesystem.read.external`、`network` 和 `process.handoff` 逐次确认；`auto` 每次签发与调用绑定的运行时授权；`deny` 直接拒绝。
...
现有 Schema v2/v3 配置可用 `agent config migrate --workspace . --check` 检查，再用 `agent config migrate --workspace . --write` 原子升级；原文件保留为 `.agent/config.toml.v2.bak` 或 `.v3.bak`。工作区配置只能收紧这些能力，不能放宽更严格的 Home 策略。单次运行对应参数为 `--read-scope workspace --network none --process-handoff deny`。
