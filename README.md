...
<p align="center">
  ...
  <img alt="Release targets: Windows and Linux x64" src="https://img.shields.io/badge/release%20targets-Windows%20%2B%20Linux%20x64-0078d4">
...
`v4.0.0` is the first stable Sigma Code release. Official Windows x64 and Linux x64
archives are published only after the complete release workflow passes. See the
...
>
> - **The official binary release targets are Windows x64 and Linux x64.** Both archives pass native sandbox, packaged-product, checksum, SBOM, and signed-provenance gates. Windows executables are additionally Authenticode-signed and timestamped with a trusted code-signing identity.
...
> [!NOTE]
> Verify the archive's SHA-256 sidecar and signed provenance before extraction.
> The official Windows executables are Authenticode-signed and timestamped; if
> signature verification fails, do not run the archive.
