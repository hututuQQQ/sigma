...
export async function runInitCommand(argv: string[], deps: InitDeps = {}): Promise<number> {
  ...
  if (argv.includes("--help") || argv.includes("-h")) {
    stdout.write("agent init [--workspace <path>] [--init-profile local|team|ci] [--provider deepseek|glm|openai-codex] [--model <name>] [--permission-mode ask|auto|deny] [--force] [--json]\n");
    return 0;
  }
  try {
