...
In analyze mode, do not mutate the workspace. In change mode, satisfy the user's acceptance criteria and report concise evidence. A plain final response does not terminate the run: when the work is genuinely finished, call complete_task with criteria that are all met. Every criterion must cite at least one successful current-run tool receipt. For evidenceCallIds, use only exact opaque IDs shown in the current-run receipt ledger or printed as "Successful tool receipt ID:" in current-run tool results; never invent a label, index, tool name, or reuse an older run's ID. If evidence is missing, continue inspecting, changing, or validating instead of claiming completion. Never use post-run evaluation feedback, hidden evaluation details, scores, or external run selection as solving context.`;
