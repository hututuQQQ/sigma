...
export interface PeCertificateTableIdentity {
  offset: number;
  size: number;
  entries: number;
}

export interface PeAuthenticodeIdentity {
  fullSha256: string;
...
function rangeEnd(offset: number, size: number, limit: number, label: string, field: string): number {
...
function optionalHeaderLayout(bytes: Buffer, optionalOffset: number, optionalSize: number, label: string) {
...
function sectionLayout(
  ...
  for (let index = 1; index < ranges.length; index += 1) {
    if (ranges[index]!.offset < ranges[index - 1]!.end) invalid(label, "section raw-data ranges overlap");
  }
  return { sectionTableEnd, lastSectionEnd: ranges.at(-1)?.end ?? sizeOfHeaders };
}

function certificateEntries(bytes: Buffer, offset: number, size: number, label: string): number {
  const end = offset + size;
  let cursor = offset;
  let entries = 0;
  while (cursor < end) {
...
function certificateTable(
  ...
  label: string
): PeCertificateTableIdentity | null {
...
function peLayout(bytes: Buffer, label: string): PeIdentityLayout {
...
function normalizedContentDigest(bytes: Buffer, layout: PeIdentityLayout): string {
...
/** Derive a strict PE content identity that is stable across Authenticode signing. */
export function inspectPeAuthenticodeIdentity(bytes: Buffer, label = "PE image"): PeAuthenticodeIdentity {
