...
const ATTESTATION_KEYS = new Set([
  ...
  "imageId", "imageDigest", "labelsDigest", "helperDigest", "attestationDigest",
...
function managedEnvironmentProof(value: unknown): TrustedManagedContainerAttestationV1["managedEnvironment"] {
  ...
  const keys = new Set([
    "protocolVersion", "targetAttestationDigest", "targetId", "targetStartedAt",
    "rootKind", "effectiveNetwork", "disposable", "protectedPaths", "proofDigest"
  ]);
  const unknown = Object.keys(proof).find((key) => !keys.has(key));
  if (unknown || !Array.isArray(proof.protectedPaths)
    || proof.protectedPaths.some((item) => typeof item !== "string")) {
    throw new ContainerAttestationInvalidError(
      "Fixed OCI managed environment proof is invalid."
    );
  }
  return {
    protocolVersion: proof.protocolVersion as 1,
    targetAttestationDigest: requiredString(
      proof.targetAttestationDigest, "managedEnvironment.targetAttestationDigest"
    ),
    targetId: requiredString(proof.targetId, "managedEnvironment.targetId"),
    targetStartedAt: requiredString(
      proof.targetStartedAt, "managedEnvironment.targetStartedAt"
    ),
    rootKind: proof.rootKind as "container_cow",
    effectiveNetwork: proof.effectiveNetwork as "full",
    disposable: proof.disposable as true,
    protectedPaths: proof.protectedPaths as string[],
    proofDigest: requiredString(proof.proofDigest, "managedEnvironment.proofDigest")
  };
}
...
export function parseFixedContainerAttestation(source: string): FixedAttestationDocument {
  ...
  const managedEnvironment = managedEnvironmentProof(value.managedEnvironment);
