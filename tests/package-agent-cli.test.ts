...
if [ "$2" = "version" ]; then echo '{"product":"Sigma Code","package":{"name":"agent-cli","version":"0.1.7"},"runtime":{"node":"${pinnedNodeVersion}"}}'; exit 0; fi
...
async function writePackageFixture() {
  ...
  await writeFile(
    ...
    `${JSON.stringify({ name: "sigma", version: "0.1.7", private: true, license: "MIT" })}\n`,
    "utf8"
  );
...
describe("package-agent-cli", () => {
  ...
  it("can explicitly verify the Linux wrapper through WSL on Windows hosts", () => {
    ...
    const spawnSync = (command: string, args: string[]) => {
      ...
      if (command === "wsl" && args[0] === "-e" && args[3]?.includes("./bin/agent version --json")) {
        return {
          status: 0,
          stdout: `${JSON.stringify({ product: "Sigma Code", package: { name: "agent-cli", version: "0.1.5" } })}\n`,
          stderr: ""
        };
      }
  ...
  it("verifies the Linux wrapper through a fixed Docker image by default on Windows", () => {
    ...
    expect(report).toMatchObject({
      ...
      version: { product: "Sigma Code", package: { name: "agent-cli", version: "0.1.5" } }
    });
