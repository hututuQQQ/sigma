...
impl DisposableWorkspace {
    ...
    /// Resolve the object that occupies a policy-authorized cwd after this
    /// disposable workspace is mounted at its original destination. The
    /// policy cwd itself must remain pinned to the real workspace for scope
    /// checks; only the hardened launcher's runtime inode check uses this
    /// mirror path.
    pub(crate) fn runtime_cwd_source(&self, policy_cwd: &Path) -> Result<PathBuf, RpcError> {
        let requested = policy_cwd;
        let policy_cwd = policy_cwd.canonicalize().map_err(|error| {
            RpcError::new(
                "validation_disposable_workspace_unavailable",
                format!(
                    "cannot resolve validation cwd '{}': {error}",
                    requested.display()
                ),
            )
        })?;
        let relative = policy_cwd.strip_prefix(&self.destination).map_err(|_| {
            RpcError::new(
                "validation_disposable_workspace_unavailable",
                format!(
                    "validation cwd '{}' is outside disposable workspace '{}'",
                    requested.display(),
                    self.destination.display()
                ),
            )
        })?;
        let candidate = self.source.join(relative);
        let resolved = candidate.canonicalize().map_err(|error| {
            RpcError::new(
                "validation_disposable_workspace_unavailable",
                format!(
                    "cannot resolve validation cwd in the disposable workspace '{}': {error}",
                    candidate.display()
                ),
            )
        })?;
        let source = self.source.canonicalize().map_err(|error| {
            RpcError::new(
                "validation_disposable_workspace_unavailable",
                format!(
                    "cannot resolve disposable validation workspace '{}': {error}",
                    self.source.display()
                ),
            )
        })?;
        let metadata = std::fs::metadata(&resolved).map_err(|error| {
            RpcError::new(
                "validation_disposable_workspace_unavailable",
                format!(
                    "cannot inspect validation cwd in the disposable workspace '{}': {error}",
                    resolved.display()
                ),
            )
        })?;
        if !resolved.starts_with(&source) || !metadata.is_dir() {
            return Err(RpcError::new(
                "validation_disposable_workspace_unavailable",
                format!(
                    "validation cwd '{}' does not map to a contained directory in the disposable workspace",
                    requested.display()
                ),
            ));
        }
...
mod tests {
    ...
    fn disposable_workspace_copies_inputs_excludes_metadata_and_cleans_up() {
        ...
        std::fs::create_dir_all(workspace.join("nested/cwd")).unwrap();
        ...
        assert_eq!(
            disposable
                .runtime_cwd_source(&workspace.join("nested/cwd"))
                .unwrap(),
            copy.join("nested/cwd").canonicalize().unwrap()
        );
        let outside = workspace.parent().unwrap().join("outside-cwd");
        std::fs::create_dir_all(&outside).unwrap();
        let error = disposable.runtime_cwd_source(&outside).unwrap_err();
    ...
    #[test]
    fn disposable_workspace_rejects_missing_and_non_directory_runtime_cwds() {
        let lease = ScratchLease::new("validation-cwd-errors");
        lease.prepare().unwrap();
        let workspace = lease.root.join("workspace-source");
        std::fs::create_dir(&workspace).unwrap();
        std::fs::write(workspace.join("file-cwd"), b"not a directory").unwrap();
        let disposable = lease.disposable_workspace(&workspace).unwrap();

        for cwd in [workspace.join("missing"), workspace.join("file-cwd")] {
            let error = disposable.runtime_cwd_source(&cwd).unwrap_err();
            assert_eq!(error.code, "validation_disposable_workspace_unavailable");
        }
    }
}
