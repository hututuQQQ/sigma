...
export function subjectNodeLaunch(subject) {
  ...
  if (!record(launch) || launch.kind !== "node" || launch.shell !== false
    || !path.isAbsolute(launch.executablePath) || !path.isAbsolute(launch.entryPath)) {
    throw new Error("CLI subject must provide an explicit direct Node launch descriptor.");
  }
  const expectedRuntime = subject.subjectKind === "package" ? "bundled" : "host-dev";
  ...
  if (typeof subject.nodePath !== "string" || typeof subject.cliEntry !== "string"
    ...
    || !samePath(subject.cliEntry, launch.entryPath, targetPlatform)) {
...
export async function loadPackagedSubjectLaunch(bundleRoot, options = {}) {
  ...
  const readBundleJson = async (relativePath, label) => {
  ...
  if (metadata.value.targetPlatform !== targetPlatform || metadata.value.targetArch !== targetArch) {
    throw new Error("Packaged subject metadata target does not match the evaluator host target.");
  }
  if (metadata.value.node?.targetPlatform !== undefined && metadata.value.node.targetPlatform !== targetPlatform) {
    throw new Error("Packaged Node target platform disagrees with package metadata.");
  }
  if (metadata.value.node?.targetArch !== undefined && metadata.value.node.targetArch !== targetArch) {
    throw new Error("Packaged Node target architecture disagrees with package metadata.");
  }
  ...
  if (integrity.value.schemaVersion !== 1 || integrity.value.algorithm !== "sha256"
    || integrity.value.targetPlatform !== targetPlatform || integrity.value.targetArch !== targetArch
    || !Array.isArray(integrity.value.entries)) {
    throw new Error("Package integrity manifest target or schema is invalid.");
  }
  const entries = new Map();
  for (const entry of integrity.value.entries) {
    const relative = normalizeBundleRelative(entry?.path, "Integrity entry");
    if (entries.has(relative) || !/^[a-f0-9]{64}$/u.test(String(entry?.sha256 ?? ""))
      || !Number.isSafeInteger(entry?.size) || entry.size < 0) {
      throw new Error("Package integrity manifest contains an invalid or duplicate entry.");
    }
    entries.set(relative, entry);
  }

  const nodeRelative = normalizeBundleRelative(
    metadata.value.node?.path ?? `bin/${targetPlatform === "win32" ? "node.exe" : "node"}`,
    "Bundled Node runtime"
  );
  const cliRelative = "packages/agent-cli/dist/index.js";
  const brokerRelative = normalizeBundleRelative(metadata.value.sigmaExec?.path, "Bundled sigma-exec broker");
  const manifestedFile = async (relative, label) => {
    const declared = entries.get(relative);
    if (!declared) throw new Error(`${label} is not covered by the package integrity manifest.`);
    const loaded = await regularBundleFile(bundleRoot, canonicalRoot, relative, label);
  ...
  const [node, cli, broker, wrapper] = await Promise.all([
    manifestedFile(nodeRelative, "Bundled Node runtime"),
    manifestedFile(cliRelative, "Bundled CLI entry"),
    manifestedFile(brokerRelative, "Bundled sigma-exec broker"),
    manifestedFile(declaredBin, "Declared package executable")
  ]);
  ...
  if ((metadataCompatibility !== undefined || manifestCompatibility !== undefined)
    && digest(metadataCompatibility ?? null) !== digest(manifestCompatibility ?? null)) {
    throw new Error("Node compatibility metadata disagrees with the integrity manifest.");
  }
  const environment = assertNodeRuntimeEnvironment(metadataCompatibility?.runtimeEnvironment, targetPlatform);
  const binDirectory = path.dirname(node.filePath);
  const launch = {
    kind: "node",
    runtime: "bundled",
    targetPlatform,
    executablePath: node.filePath,
    entryPath: cli.filePath,
    declaredExecutablePath: wrapper.filePath,
    shell: false,
    binDirectory,
    pathDelimiter: targetPlatform === "win32" ? ";" : ":",
    environment
  };
  return {
    nodePath: node.filePath,
    cliEntry: cli.filePath,
    brokerPath: broker.filePath,
    launch,
    packageMetadataDigest: digest(metadata.bytes),
    integrityManifestDigest: digest(integrity.bytes)
  };
}
